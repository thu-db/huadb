...
NewPageLog::NewPageLog(xid_t xid, lsn_t prev_lsn, oid_t oid, oid_t db_oid, pageid_t prev_page_id, pageid_t page_id)
    : LogRecord(LogType::NEW_PAGE, xid, prev_lsn),
      oid_(oid),
      db_oid_(db_oid),
      prev_page_id_(prev_page_id),
      page_id_(page_id) {
  size_ += sizeof(oid_) + sizeof(db_oid_) + sizeof(page_id_) + sizeof(prev_page_id_);
}
...
size_t NewPageLog::SerializeTo(char *data) const {
  ...
  offset += sizeof(oid_);
  memcpy(data + offset, &db_oid_, sizeof(oid_));
...
std::shared_ptr<NewPageLog> NewPageLog::DeserializeFrom(const char *data) {
  ...
  offset += sizeof(oid);
  memcpy(&db_oid, data + offset, sizeof(oid));
  ...
  return std::make_shared<NewPageLog>(xid, prev_lsn, oid, db_oid, prev_page_id, page_id);
}
